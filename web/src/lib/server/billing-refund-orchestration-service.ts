...
async function processRefundClaim(claim: RefundClaim, workerId: string) {
    ...
    try {
        ...
        if (providerRefund.status === "pending") {
            const status = nextRetryStatus(claim.job);
            await releaseJob(claim.job, workerId, status, undefined, undefined, providerRefund, payload);
            await markOrderRefundDeferred(claim.order, "等待支付商确认退款结果", status);
            return { order: claim.order, providerRefund, pending: status !== "manual", manual: status === "manual" };
        }
...
function refundJobPayload(current: JsonValue | undefined, patch: { reason?: string; operatorUserId?: string; providerRefund?: PaymentRefundResult }) {
    ...
    const providerRefundMetadata = patch.providerRefund ? jsonObject(paymentRefundMetadata(patch.providerRefund, false)) : undefined;
    return sanitizeJson({
        ...source,

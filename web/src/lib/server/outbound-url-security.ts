...
export type SafeOutboundOptions = {
    ...
    privateHostnames?: Iterable<string>;
};
...
export async function resolveSafeOutboundTarget(value: string | URL, options?: SafeOutboundOptions): Promise<SafeOutboundTarget | null> {
    ...
    const privateAllowed = explicitPrivateHostAllowed(hostname, options?.privateHostnames) || (options?.allowPrivateUpstreams !== false && privateUpstreamHostAllowed(hostname));
    if ((hostname === "localhost" || hostname.endsWith(".localhost")) && !privateAllowed) return null;
    const directFamily = isIP(hostname);
    if (directFamily) return addressAllowed(hostname, privateAllowed, false) ? { url, hostname, address: hostname, family: directFamily as 4 | 6 } : null;

    try {
        const addresses = dedupeAddresses(await lookup(hostname, { all: true, verbatim: true }));
        if (!addresses.length || !addresses.every((item) => addressAllowed(item.address, privateAllowed, true))) return null;
...
export async function isSafeOutboundUrl(value: string | URL, options?: SafeOutboundOptions) {
...
function addressAllowed(address: string, privateAllowed: boolean, resolvedHostname: boolean) {
    if (isPublicIpAddress(address)) return true;
    if (privateAllowed && isPrivateAllowlistableAddress(address)) return true;
    return resolvedHostname && process.env.DQ_ALLOW_FAKE_IP_DNS === "1" && isClashFakeIpAddress(address);
}
...
function privateUpstreamHostAllowed(hostname: string) {
    ...
    return (process.env.DQ_PRIVATE_UPSTREAM_HOSTS || "").split(",").map(normalizeHostname).filter(Boolean).includes(hostname);
}

function explicitPrivateHostAllowed(hostname: string, privateHostnames?: Iterable<string>) {
    if (!privateHostnames) return false;
    return Array.from(privateHostnames, normalizeHostname).filter(Boolean).includes(hostname);
}

...
describe("payment provider checkout readiness", () => {
    ...
    it("accepts an enabled provider with production checkout credentials", () => {
        expect(isPaymentRuntimeProviderCheckoutReady(config({ stripe: { enabled: true, saved: true } }, { DQ_STRIPE_SECRET_KEY: "sk_live_real_key" }), "stripe")).toBe(true);
    });
...
function alipayValues(mode?: string) {
    return {
        ...(mode ? { DQ_ALIPAY_MODE: mode } : {}),
        DQ_ALIPAY_APP_ID: "2026000000000000",
        DQ_ALIPAY_PRIVATE_KEY: "private-key-data",
    };
}

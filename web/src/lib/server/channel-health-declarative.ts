...
export async function testDeclarativeChannelProtocol(baseUrl: string, apiKey: string, model: string, kind: ChannelHealthKind, protocol: SystemChannelProtocol, advanced: SystemChannelAdvancedConfig): Promise<ChannelHealthResult> {
    ...
    const prompt = kind === "text" ? "Reply exactly OK." : kind === "image" ? "A single blue circle icon on a white background." : kind === "video" ? "A calm 4 second shot of a blue circle logo on a white background." : "DQ-绘图 audio health check.";
    const values = {
        model,
        prompt,
        input: prompt,

...
describe("system AI proxy policy", () => {
    it("allows a billed create path for the bound logical model", () => {
        expect(
            authorizeSystemAiProxyRequest({ method: "POST", path: ["chat", "completions"], search: "", channelId: "main", upstreamModel: "vendor-text", preferredLogicalModelId: "writer", logicalModels, apiFormat: "openai", pointsUsageKind: "text" }),
        ).toMatchObject({
            allowed: true,
    ...
    it("rejects unsupported methods and mismatched logical capabilities", () => {
        expect(authorizeSystemAiProxyRequest({ method: "PUT", path: ["chat", "completions"], search: "", channelId: "main", upstreamModel: "vendor-text", logicalModels, apiFormat: "openai", pointsUsageKind: "text" })).toMatchObject({
            allowed: false,
            status: 405,
        });
        expect(authorizeSystemAiProxyRequest({ method: "POST", path: ["responses"], search: "", channelId: "main", upstreamModel: "vendor-text", logicalModels, apiFormat: "openai", pointsUsageKind: "image" })).toMatchObject({
            allowed: false,
            status: 403,
        });
    });

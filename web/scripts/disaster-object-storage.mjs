...
async function readPostgresSettings(databaseUrl) {
    ...
    try {
        const result = await client.query("SELECT * FROM dq_object_storage_settings WHERE id = 'default'");
...
function decryptSecret(value, rawKey) {
    ...
    if (!key) throw new Error("对象存储已启用，但 DQ_ENCRYPTION_KEY 缺失或格式不正确");

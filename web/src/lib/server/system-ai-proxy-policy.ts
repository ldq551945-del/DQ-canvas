...
const TASK_PARAMETER = "__DQ_TASK_PARAMETER__";
...
export function authorizeSystemAiProxyRequest(input: ProxyPolicyInput): SystemAiProxyAccess {
    ...
    const cancelMatch =
        method === "POST" || method === "DELETE"
            ...
            : null;
    if (cancelMatch) return allowedTaskOperation(logical, "cancel", taskIdForAccess(cancelMatch.taskId, input.upstreamTaskIdHint));
    ...
    if (queryMatch) return allowedTaskOperation(logical, "query", taskIdForAccess(queryMatch.taskId, input.upstreamTaskIdHint));
...
function defaultQueryPaths(capability: LogicalModelCapability, createPaths: Array<string | undefined>) {
    ...
    if (capability === "video") return [...fromCreate, "/videos/:task_id", "/video/generations/:task_id", "/videos/generations/:task_id", "/result?id=:task_id", "/agnesapi?video_id=:task_id", "/v1/videos/:task_id/content", "/videos/:task_id/content"];
...
function defaultCancelPaths(capability: LogicalModelCapability, createPaths: Array<string | undefined>) {
    ...
    if (capability === "video") paths.push({ path: "/videos/:task_id/cancel", method: "POST" }, { path: "/video/generations/:task_id/cancel", method: "POST" });

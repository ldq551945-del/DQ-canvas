...
const originalKey = process.env.DQ_ENCRYPTION_KEY;

afterEach(() => {
    if (originalKey === undefined) delete process.env.DQ_ENCRYPTION_KEY;
    else process.env.DQ_ENCRYPTION_KEY = originalKey;
});

describe("secret encryption key", () => {
    it("encrypts and decrypts with a 32-byte hexadecimal key", () => {
        process.env.DQ_ENCRYPTION_KEY = "ab".repeat(32);
    ...
    it("accepts an exact 32-byte base64 key", () => {
        process.env.DQ_ENCRYPTION_KEY = Buffer.alloc(32, 7).toString("base64");
    ...
    it("rejects missing, placeholder and passphrase values instead of storing plaintext", () => {
        for (const key of ["", "change-this-random-secret", "replace-with-a-long-random-secret", "ordinary-passphrase"]) {
            process.env.DQ_ENCRYPTION_KEY = key;

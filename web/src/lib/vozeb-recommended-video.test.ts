import { describe, expect, it } from "vitest";

import { assertVozebRecommendedVideoReferences, buildVozebRecommendedVideoRequest } from "./vozeb-recommended-video";

describe("VOZEB recommended video protocol", () => {
    it("forces the fast 720p model to disable generated audio", () => {
        expect(
            buildVozebRecommendedVideoRequest({
                model: "Seedance 2.0-fast-720p",
                prompt: "test",
                duration: 5,
                aspectRatio: "16:9",
                resolution: "1080p",
                generateAudio: true,
                images: [],
                videos: [],
                audios: [],
            }),
        ).toMatchObject({ resolution: "720p", generate_audio: false });
    });

    it("keeps supported references in bounded JSON arrays", () => {
        expect(
            buildVozebRecommendedVideoRequest({
                model: "qy-seedance-2.0-fast",
                prompt: "test",
                duration: 10,
                aspectRatio: "9:16",
                resolution: "720p",
                generateAudio: true,
                images: ["image-one"],
                videos: ["video-one"],
                audios: ["audio-one"],
            }),
        ).toEqual({
            model: "qy-seedance-2.0-fast",
            prompt: "test",
            duration: 10,
            resolution: "720p",
            generate_audio: true,
            aspect_ratio: "9:16",
            images: ["image-one"],
            videos: ["video-one"],
            audios: ["audio-one"],
        });
    });

    it("rejects unsupported video and audio references for the fast 720p model", () => {
        expect(() => assertVozebRecommendedVideoReferences("Seedance 2.0-fast-720p", [{ type: "video" }])).toThrow("不支持参考视频");
        expect(() => assertVozebRecommendedVideoReferences("models/Seedance 2.0-fast-720p", [{ type: "audio" }])).toThrow("不支持参考音频");
    });
});

...
export const E2E_ADMIN = {
    ...
    password: "DqE2E!2026",
    displayName: "E2E 管理员",
    installToken: "dq-e2e-install-token-more-than-32-characters",
};

export const E2E_PROTOCOL_ORIGIN = `http://127.0.0.1:${Number(process.env.DQ_PROTOCOL_FIXTURE_PORT || 4010)}`;
export const E2E_PAYMENT_WEBHOOK_SECRET = "dq-e2e-payply-webhook-secret-more-than-32-characters";

const models = ["e2e-text", "e2e-text-fallback", "e2e-text-fail", "e2e-image", "e2e-image-fallback", "e2e-image-fail", "e2e-video", "e2e-video-fallback", "e2e-video-slow", "e2e-audio", "e2e-audio-fallback"];
...
export function e2eSettingsPatch() {
    ...
    const channels = [channel("e2e-primary", "E2E 主渠道", "e2e-primary-secret", modelCapabilities, modelConfigs), channel("e2e-backup", "E2E 备用渠道", "e2e-backup-secret", modelCapabilities, modelConfigs)];
    const logicalModels = models.map((model) => ({
        id: model,
        name: model,
        capability: modelCapabilities[model],
        enabled: true,
        bindings: channels.map((item, index) => ({ id: `${item.id}:${model}`, channelId: item.id, upstreamModel: model, enabled: true, priority: index + 1 })),
    }));
    return {

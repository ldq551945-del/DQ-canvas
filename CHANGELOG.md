...
- [Agent] 文本规划兼容 Chat、Responses、Gemini 和声明式自定义协议，统一 3 分钟候选超时并自动切换健康绑定；规划输入按任务复杂度裁剪，多结果逐份推送，内部规划与复盘内容不再进入用户可见消息。
- [生成] 新增持久生成 Worker、任务租约、心跳就绪检查、HMAC 回调和 PostgreSQL 跨实例事件通知；视频等待上限为 30 分钟，查询异常和本地等待结束继续复用原上游任务，只有上游明确失败才允许显式重试；页面关闭或实例切换后继续恢复，人工确认不占用生成并发。
- [上游] 后台补齐渠道管理、协议中心、逻辑模型、验证记录和接入向导，支持 OpenAI/Gemini 目录变体、Seedance 2.0、Stable Diffusion、A1111/Forge 及自定义创建/查询/取消协议；Seedance 2.0 特价协议支持自身内容端点回退且不改变官方 NewAPI，同名模型跨渠道合并绑定，关闭渠道时自动修正失效默认模型。
...
- [工程] 移动端工作区导航在路由变化后可靠关闭；production 构建将严格类型检查与 Next.js 构建拆为独立进程以降低 1GB 堆环境峰值，并使用独立 `.next-production` 与统一 standalone 资源装配；Compose 与 Render Blueprint 增加主应用、Worker、数据库、维护令牌、健康检查和持久卷结构化契约。

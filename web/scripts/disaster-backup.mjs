...
try {
    ...
    const databaseFile = "database/dq.dump";
    const databasePath = path.join(recoveryPointDir, databaseFile);
    await mkdir(path.dirname(databasePath), { recursive: true });
    await runPostgresTool(process.env.DQ_PG_DUMP_PATH || "pg_dump", ["--format=custom", "--compress=6", "--no-owner", "--no-privileges", "--file", databasePath], databaseUrl);
    ...
    for (const name of LOCAL_MEDIA_ROOTS) {
        ...
        localMediaRoots.push({ name, present: snapshot.present, files: snapshot.files.map((file) => ({ ...file, file: `local-media/${name}/${file.path}` })) });
    }
    ...
    const manifest = {
        ...
        consistency: "offline",
        source: { database: publicDatabaseIdentity(databaseUrl), dataDirectory: dataDir },
        database,
        localMedia: { roots: localMediaRoots },
        objectStorage,
        requiredSecrets: ["DATABASE_URL", "DQ_ENCRYPTION_KEY"],
    };

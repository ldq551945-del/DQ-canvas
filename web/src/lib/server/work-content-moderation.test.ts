...
describe("work content moderation", () => {
    ...
    it("returns a review signal when configured keywords match", async () => {
        const result = await moderateWorkContent({ ...input, description: "包含风险词" }, { env: { DQ_CONTENT_MODERATION_PROVIDER: "keywords", DQ_CONTENT_MODERATION_KEYWORDS: "风险词,另一个词" }, now });
    ...
    it("checks the explicitly public prompt", async () => {
        const result = await moderateWorkContent({ ...input, publicPrompt: "公开风险词" }, { env: { DQ_CONTENT_MODERATION_PROVIDER: "keywords", DQ_CONTENT_MODERATION_KEYWORDS: "风险词" }, now });
    ...
    it("falls back to manual review when an external provider fails", async () => {
        ...
        const result = await moderateWorkContent(input, { env: { DQ_CONTENT_MODERATION_PROVIDER: "http", DQ_CONTENT_MODERATION_URL: "https://moderation.example.com/check" }, fetchImpl, now });

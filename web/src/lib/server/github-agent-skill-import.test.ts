...
describe("github agent skill import", () => {
    ...
    it("rejects a truncated GitHub tree instead of silently importing incomplete candidates", async () => {
        vi.stubGlobal(
            "fetch",
            vi.fn(async (input: string | URL) => {
                const url = String(input);
                if (url.endsWith("/repos/acme/large-library")) return response({ default_branch: "main" });
                if (url.endsWith("/repos/acme/large-library/commits/main")) return response({ sha: commit });
                if (url.includes(`/git/trees/${commit}?recursive=1`)) return response({ truncated: true, tree: [{ type: "blob", path: "one/SKILL.md" }] });
                return response({ message: "not found" }, 404);
            }),
        );

        await expect(importAgentSkillFromGithub({ url: "https://github.com/acme/large-library" })).rejects.toMatchObject({ status: 413 });
    });

...
describe("/api/drama/projects", () => {
    beforeEach(() => {
        ...
        mocks.listDramaProjectSummariesForUser.mockResolvedValue([]);
    ...
    it("returns lightweight summaries for the current user", async () => {
        mocks.listDramaProjectSummariesForUser.mockResolvedValue([{ id: "drama-one", title: "测试短剧", episodeCount: 2, shotCount: 12 }]);

        const response = await GET();

        expect(response.status).toBe(200);
        expect(mocks.listDramaProjectSummariesForUser).toHaveBeenCalledWith("user-one");
        await expect(response.json()).resolves.toMatchObject({ data: { projects: [{ id: "drama-one", episodeCount: 2, shotCount: 12 }] } });
    });
});

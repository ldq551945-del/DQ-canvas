...
describe("reference asset access", () => {
    it("creates and verifies a bounded signed server URL", () => {
        ...
        expect(verifyReferenceAssetSignature("temporary/2026/07/19/images/file.png", url.searchParams.get("expires"), url.searchParams.get("signature"), now)).toBe(true);
        expect(verifyReferenceAssetSignature("temporary/2026/07/19/images/other.png", url.searchParams.get("expires"), url.searchParams.get("signature"), now)).toBe(false);
        expect(verifyReferenceAssetSignature("temporary/2026/07/19/images/file.png", url.searchParams.get("expires"), url.searchParams.get("signature"), now + 16 * 60 * 1000)).toBe(false);
    });
    ...
    it("only signs local reference asset paths", () => {
        ...
        expect(signReferenceAssetInputUrl("/api/reference-assets/permanent/2026/07/19/images/file.png", "https://vozeb.example")).toContain("signature=");
    });
});

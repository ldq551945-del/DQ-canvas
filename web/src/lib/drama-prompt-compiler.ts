...
export function compileDramaShotPrompts(project: DramaProject, episode: DramaEpisode, shot: DramaShot): CompiledDramaPrompts {
    ...
    const startFramePrompt = limit(compact([...shared, `起始动作状态：${shot.continuity?.actionStart || shot.description}`, shot.startFramePrompt || shot.imagePrompt]).join("\n"));
    const endFramePrompt = limit(compact([...shared, `结束动作状态：${shot.continuity?.actionEnd || shot.description}`, shot.endFramePrompt || shot.videoPrompt || shot.imagePrompt]).join("\n"));
    const videoPrompt = limit(
        compact([
            ...shared,
            `镜头事实：${shot.description || shot.sourceText}`,

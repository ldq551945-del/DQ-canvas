import { closestImageAspectRatio, normalizeImageSizeValue } from "@/lib/image-size";
...
export function resolvePortraitTextureSize(sourceSize: unknown, naturalWidth: number | undefined, naturalHeight: number | undefined, fallbackSize: unknown) {
    return normalizeImageSizeValue(sourceSize) || closestImageAspectRatio(naturalWidth, naturalHeight) || normalizeImageSizeValue(fallbackSize) || "auto";
}

export function resolvePortraitTextureRetryState(input: { resultSettings?: unknown; sourceSettings?: unknown; resultBasePrompt?: string; fallbackBasePrompt?: string }) {
    const settingsValue = input.resultSettings ?? input.sourceSettings;
    const settings = settingsValue === undefined ? undefined : normalizePortraitTextureSettings(settingsValue);
    const basePrompt = (input.resultBasePrompt || input.fallbackBasePrompt || "").trim();
    return {
        basePrompt,
        settings,
        prompt: settings ? buildPortraitTexturePrompt(basePrompt, settings) : basePrompt,
    };
}

...
const previousKey = process.env.DQ_REFERENCE_ASSET_SIGNING_KEY;

afterEach(() => {
    if (previousKey === undefined) delete process.env.DQ_REFERENCE_ASSET_SIGNING_KEY;
    else process.env.DQ_REFERENCE_ASSET_SIGNING_KEY = previousKey;
});

describe("reference asset access", () => {
    it("creates and verifies a bounded signed server URL", () => {
        process.env.DQ_REFERENCE_ASSET_SIGNING_KEY = "test-signing-key";
        const now = Date.UTC(2026, 6, 19);
        const url = new URL(createSignedReferenceAssetUrl("temporary/2026/07/19/images/file.png", "https://dq.example", now));
        const purpose = url.searchParams.get("purpose");

        expect(url.origin).toBe("https://dq.example");
    ...
    it("only signs local reference asset paths", () => {
        process.env.DQ_REFERENCE_ASSET_SIGNING_KEY = "test-signing-key";
        expect(signReferenceAssetInputUrl("https://cdn.example/image.png", "https://dq.example")).toBe("https://cdn.example/image.png");
        expect(signReferenceAssetInputUrl("/api/reference-assets/permanent/2026/07/19/images/file.png", "https://dq.example")).toContain("purpose=provider-read");
    });
});

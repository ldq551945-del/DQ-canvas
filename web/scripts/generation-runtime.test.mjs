...
describe("generation runtime environment", () => {
    it("uses one configured maintenance token for the app and worker", () => {
        ...
        const result = generationRuntimeEnvironment({ environment: { DQ_MAINTENANCE_TOKEN: token, PORT: "3100" } });

        expect(result).toMatchObject({ ephemeralToken: false, environment: { DQ_MAINTENANCE_TOKEN: token, DQ_WORKER_API_ORIGIN: "http://127.0.0.1:3100" } });
    });
    ...
    it("generates a process-local token only for development", () => {
        ...
        expect(result.environment.DQ_MAINTENANCE_TOKEN).toHaveLength(64);
    });

    it("fails production startup before the app can run without a valid token", () => {
        expect(() => generationRuntimeEnvironment({ environment: { DQ_MAINTENANCE_TOKEN: "short" } })).toThrow("at least 32 characters");
    });

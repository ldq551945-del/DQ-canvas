...
export function normalizeCreativeFoundation(value: unknown, fallbackObjective: string): CreativeFoundation {
    ...
    const objective = text(brief.objective, 1000) || text(fallbackObjective, 1000) || "完成当前创作需求";
    return {
        complexity: record.complexity === "complex" ? "complex" : "simple",
        brief: compact({
            objective,
            audience: text(brief.audience, 500),
            usage: text(brief.usage, 500),
            coreMessage: text(brief.coreMessage, 800),
            constraints: strings(brief.constraints, 12, 300),
            referenceStrategy: text(brief.referenceStrategy, 800),
        }),
        direction: compact({
            summary: text(direction.summary, 1000) || "保持主体、信息和视觉语言清晰统一",
            style: text(direction.style, 500),
            composition: text(direction.composition, 800),
            colors: strings(direction.colors, 12, 80),
            lighting: text(direction.lighting, 500),
            keywords: strings(direction.keywords, 16, 100),
            avoid: strings(direction.avoid, 12, 200),
        }),
    };
}
...
export function normalizeCreativeDeliverables(value: unknown, fallback: CreativeDeliverableSummary): CreativeDeliverableSummary[] {
    ...
    const deliverables = items.slice(0, 8).flatMap((item) => {
        const record = object(item);
        const type = text(record.type, 16) as CreativeMediaType;
        const title = text(record.title, 160);
        const role = text(record.role, 400);
...
export function normalizeCreativeReview(value: unknown, validTaskIds: Set<string> = new Set()): CreativeReview | null {
    ...
    const summary = text(record.summary, 1200);
    ...
    const issues = (Array.isArray(record.issues) ? record.issues : []).slice(0, 8).flatMap((item) => {
        const issue = object(item);
        const category = text(issue.category, 80);
        const message = text(issue.message, 500);
        if (!category || !message) return [];
        const taskId = text(issue.taskId, 120);
        const severity: CreativeReviewIssue["severity"] = issue.severity === "high" || issue.severity === "medium" ? issue.severity : "low";
        const normalized: CreativeReviewIssue = compact({ taskId: taskId && (!validTaskIds.size || validTaskIds.has(taskId)) ? taskId : "", category, severity, message, correction: text(issue.correction, 500) });
        return [normalized];
    });
    const actionableTaskIds = new Set(issues.filter((issue) => issue.taskId && issue.severity !== "low").map((issue) => issue.taskId as string));
    const retryTaskIds =
        status === "needs_revision"
            ? Array.from(new Set((Array.isArray(record.retryTaskIds) ? record.retryTaskIds : []).map((item) => text(item, 120)).filter((id): id is string => Boolean(id && validTaskIds.has(id) && actionableTaskIds.has(id))))).slice(0, 10)
            : [];
...
function strings(value: unknown, maxItems: number, maxLength: number) {
    return Array.isArray(value) ? Array.from(new Set(value.map((item) => text(item, maxLength)).filter(Boolean))).slice(0, maxItems) : undefined;
}

...
export async function POST(request: Request, { params }: { params: Promise<{ channelId: string }> }) {
    try {
        ...
        const verification = verifyGenerationWebhookSignature({
            channelId,
            eventId,
            timestamp: request.headers.get("x-dq-timestamp") || request.headers.get("x-timestamp") || "",
            rawBody,
            signature: request.headers.get("x-dq-signature") || request.headers.get("x-signature") || "",

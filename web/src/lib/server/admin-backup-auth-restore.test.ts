...
describe("PostgreSQL account-config auth restore", () => {
    ...
    it("uses upserts and never deletes users or backup-missing auth entities", async () => {
        const query = vi.fn(async (..._args: [string, unknown[]?]) => ({ rows: [] }));
        ...
        expect(mocks.insertPostgresUsers).toHaveBeenCalledWith(client, [expect.objectContaining({ id: "user-a" })]);
        expect(query.mock.calls.map(([sql]) => String(sql).toUpperCase()).some((sql) => sql.includes("DELETE FROM"))).toBe(false);
    });
});

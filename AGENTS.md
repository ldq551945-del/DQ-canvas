...
- 图片、视频和 Canvas Agent 默认由后台默认文本模型完成规划；用户显式选择可用的智能体文本逻辑模型时按该选择规划。规划模型应主动替用户选择能力匹配的生成逻辑模型与参数，规划结果只用于内部执行，不要求用户自行猜选生成模型。
- 修改统一 Agent 的控件、请求字段或执行行为时，必须逐项核对 `/create`、Canvas、图片工作台、视频工作台和短剧 Agent，更新所有适用入口及对应测试；不能只修改截图所在页面。用户显式选择智能体文本模型时可以覆盖后台默认文本模型，但只保存逻辑模型 ID，不得向前端暴露渠道、上游地址或 API Key。
...
- 所有默认站点品牌 Logo、ICO、favicon、manifest/metadata 图标、登录/用户端/后台品牌标识和助手头像必须使用 DQ-绘图 品牌标识，并统一从后台站点 Logo/图标配置或内置 `/logo.svg`、`/icon.svg` 派生；禁止使用旧品牌图形作为 Logo、默认占位或回退图标。播放、告警等功能性图标仍按语义使用。

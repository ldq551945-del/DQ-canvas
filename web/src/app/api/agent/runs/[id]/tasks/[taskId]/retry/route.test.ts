...
describe("Agent child task retry concurrency", () => {
    ...
    it("discards failed child task IDs before starting a new retry", async () => {
            ...
            tasks: [
                {
                    ...
                    childTasks: [{ id: "child-failed", status: "pending", attempt: 3 }],
                    error: "视频生成超时",
                },
            ],
        };
        ...
        mocks.updateAgentRunById.mockResolvedValue({ ...run, status: "running" });

        const response = await POST(new Request("http://localhost/api/agent/runs/run/tasks/task/retry", { method: "POST" }), { params: Promise.resolve({ id: "run", taskId: "task" }) });

        expect(response.status).toBe(200);
        const tasks = mocks.updateAgentRunById.mock.calls[0]?.[1]?.tasks;
        expect(tasks).toEqual([expect.objectContaining({ id: "task", status: "ready", attempts: 0, taskId: undefined, taskIds: undefined, childTasks: undefined, result: undefined, error: undefined })]);
        expect(mocks.executeAgentRun).toHaveBeenCalledWith(expect.objectContaining({ id: "run", status: "running" }), "http://localhost", "");
    });
});

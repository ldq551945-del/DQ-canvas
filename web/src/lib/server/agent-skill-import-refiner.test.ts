...
describe("agent skill import refiner", () => {
    ...
    it("rejects model output that still contains provider setup instructions and refunds charged usage", async () => {
        mocks.requestStructuredText.mockResolvedValue({
            ...
            headers: new Headers({ "x-dq-points-cost": "2", "x-dq-points-record-id": "record-1" }),
            protocol: "chat",
            elapsedMs: 10,
        });
        ...
        expect(mocks.refundUserPoints).toHaveBeenCalledWith("admin", "planner", 2, "text", 1, undefined, "record-1");
    });

    it("rejects executable web or code deliverables that DQ cannot run and refunds charged usage", async () => {
        mocks.requestStructuredText.mockResolvedValue({
            arguments: JSON.stringify({ ...refined, instructions: "1. 提炼算法艺术概念。\n2. 设计粒子与流场。\n3. 生成单个可运行的交互 HTML 应用程序。\n4. 检查构图、色彩和性能。" }),
            headers: new Headers({ "x-dq-points-cost": "2", "x-dq-points-record-id": "record-2" }),
            protocol: "chat",
            elapsedMs: 10,
        });

        await expect(refineImportedAgentSkill({ skill, requestUrl: "http://localhost/api/admin/agent-skills/import", cookie: "session=1", userId: "admin" })).rejects.toBeInstanceOf(AgentSkillRefinementError);
        expect(mocks.refundUserPoints).toHaveBeenCalledWith("admin", "planner", 2, "text", 1, undefined, "record-2");
    });
    ...
    it("normalizes only Chinese, platform-native structured output", () => {
        ...
        expect(normalizeRefinedSkill({ ...refined, instructions: "1. 提炼创作概念。\n2. 规划粒子流场。\n3. 生成可运行的交互 HTML 应用程序。\n4. 检查构图与性能。" })).toBeNull();

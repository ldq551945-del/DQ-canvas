...
describe("generation media authorization", () => {
    beforeEach(() => vi.stubEnv("DQ_ENCRYPTION_KEY", "31".repeat(32)));
    ...
    it("rejects tampered and expired capabilities", () => {
        ...
        const token = String(headers["x-dq-media-authorization"]);
        const tampered = new Request("http://localhost", { headers: { "x-dq-media-authorization": `${token}x` } });
        expect(readGenerationMediaClaim(tampered, { userId: "user", channelId: "channel", url: "https://cdn.example.com/result.png" })).toBeNull();
        const extended = new Request("http://localhost", { headers: { "x-dq-media-authorization": `${token}.ignored` } });
        expect(readGenerationMediaClaim(extended, { userId: "user", channelId: "channel", url: "https://cdn.example.com/result.png" })).toBeNull();

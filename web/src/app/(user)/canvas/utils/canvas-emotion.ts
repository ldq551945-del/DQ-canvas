...
// The model redraws only the face crop; the final output always blends into the source image.
...
export function clampFaceBox(box: CanvasFaceBox, imageWidth: number, imageHeight: number): CanvasFaceBox {
    ...
    const x = Math.max(0, Math.min(safeWidth - 1, box.x));
    const y = Math.max(0, Math.min(safeHeight - 1, box.y));
    return {
        ...box,
        x,
        y,
        width: Math.max(1, Math.min(safeWidth - x, box.width)),
        height: Math.max(1, Math.min(safeHeight - y, box.height)),
    };
}
...
function clampEditRegion(region: CanvasEmotionEditRegion, imageWidth: number, imageHeight: number): CanvasEmotionEditRegion {
    const safeWidth = Math.max(1, imageWidth);
    const safeHeight = Math.max(1, imageHeight);
    const x = Math.max(0, Math.min(safeWidth - 1, Math.round(region.x)));
    const y = Math.max(0, Math.min(safeHeight - 1, Math.round(region.y)));
    return {
        x,
        y,
        width: Math.max(1, Math.min(safeWidth - x, Math.round(region.width))),
        height: Math.max(1, Math.min(safeHeight - y, Math.round(region.height))),
    };
}

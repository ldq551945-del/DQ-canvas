...
describe("disaster recovery core", () => {
    ...
    it("copies ordinary files, rejects path traversal and verifies hashes", async () => {
        ...
        const databasePath = path.join(recovery, "database", "dq.dump");
        ...
        const manifest = {
            ...
            database: { ...(await hashFile(databasePath)), file: "database/dq.dump" },
            localMedia: {
                roots: [
...
async function temporaryDirectory() {
    const root = path.join(os.tmpdir(), `dq-disaster-test-${crypto.randomUUID()}`);

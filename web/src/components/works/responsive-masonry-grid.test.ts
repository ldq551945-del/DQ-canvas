...
describe("responsive masonry grid", () => {
    ...
    it("falls back to one column and normalizes invalid spans", () => {
        expect(masonryPlacements([0, 2.1], 0)).toEqual([
            { column: 1, rowStart: 1, rowSpan: 1 },
            { column: 1, rowStart: 2, rowSpan: 3 },
        ]);
    });
});

...
try {
    ...
    const dataDir = path.resolve(process.env.DQ_DATA_DIR || path.join(process.cwd(), ".data"));
    ...
    await runPostgresTool(process.env.DQ_PG_DUMP_PATH || "pg_dump", ["--format=custom", "--compress=6", "--no-owner", "--no-privileges", "--file", safetyDatabasePath], databaseUrl);
    ...
    await runPostgresTool(process.env.DQ_PG_RESTORE_PATH || "pg_restore", ["--clean", "--if-exists", "--no-owner", "--no-privileges", "--exit-on-error", "--single-transaction", "--dbname", postgresDatabaseName(databaseUrl), databaseDump], databaseUrl);
    await restoreLocalMedia(recoveryPointDir, dataDir, manifest);

    const objectStorageConfig = await loadDisasterObjectStorageConfig({ databaseUrl, dataDir, encryptionKey: process.env.DQ_ENCRYPTION_KEY });
...
async function restoreLocalMedia(recoveryPointDir, dataDir, manifest) {
    const stagingDir = path.join(path.dirname(dataDir), `.dq-restore-${manifest.recoveryPointId}`);
    const rollbackDir = path.join(path.dirname(dataDir), `.dq-restore-rollback-${manifest.recoveryPointId}`);

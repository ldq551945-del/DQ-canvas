...
export async function verifyPaymentTransaction(provider: string, parsed: ParsedPaymentWebhook, order: BillingOrderRecord): Promise<PaymentVerificationResult> {
    ...
    if (!queried) return { verified: false, reason: "当前支付渠道未提供可用的真实交易查询能力", payment: callback };
    const verified = withVerificationPayload(callback, queried);
    if (verified.status !== "succeeded") return { verified: false, reason: `支付商交易尚未成功：${verified.status}`, payment: verified };
    assertPaymentOrderIdentity(verified, order);
    if (!paymentCompleteForOrder(verified, order)) return { verified: false, reason: "支付商交易详情仍缺少订单身份、金额、币种或交易号", payment: verified };
    return { verified: true, payment: verified };
}
...
function withVerificationPayload(callback: VerifiedPaymentTransaction, queried: VerifiedPaymentTransaction): VerifiedPaymentTransaction {
    return {
        ...queried,
...
async function queryStripePayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction | null> {
    const secret = requiredConfig(config, "DQ_STRIPE_SECRET_KEY", "STRIPE_SECRET_KEY");
    ...
    const response = await fetchSafeOutboundUrl(`${stripeApiBase(config)}${path}`, { headers: { authorization: `Bearer ${secret}` }, signal: AbortSignal.timeout(15_000) });
    ...
    return {
        status,
        orderId: clean(
            readPath(payload, "metadata.orderId") ||
                readPath(payload, "metadata.dqOrderId") ||
                readPath(payload, "metadata.vozebProOrderId") ||
                readPath(object, "metadata.orderId") ||
                readPath(object, "metadata.dqOrderId") ||
                readPath(object, "metadata.vozebProOrderId") ||
                payload.client_reference_id,
            120,
        ),
        orderNo: clean(
            readPath(payload, "metadata.orderNo") ||
                readPath(payload, "metadata.dqOrderNo") ||
                readPath(payload, "metadata.vozebProOrderNo") ||
                readPath(object, "metadata.orderNo") ||
                readPath(object, "metadata.dqOrderNo") ||
                readPath(object, "metadata.vozebProOrderNo"),
            120,
        ),
...
async function queryAlipayPayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction> {
    const appId = requiredConfig(config, "DQ_ALIPAY_APP_ID");
    const privateKey = loadPrivateKey(config, "DQ_ALIPAY_PRIVATE_KEY", "DQ_ALIPAY_PRIVATE_KEY_PATH");
    const gateway = getPaymentRuntimeEnv(config, "DQ_ALIPAY_GATEWAY_URL") || "https://openapi.alipay.com/gateway.do";
    ...
    const response = await fetchSafeOutboundUrl(gateway, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams(params), signal: AbortSignal.timeout(15_000) });
    ...
    if (!sign || !signContent || !verifyRsaSha256(signContent, sign, loadPaymentPublicKey(config, "DQ_ALIPAY_PUBLIC_KEY", "DQ_ALIPAY_PUBLIC_KEY_PATH"))) throw new BillingInputError("支付宝交易查询响应验签失败", 502);
...
async function queryPayplyPayment(order: BillingOrderRecord, payment: VerifiedPaymentTransaction, config: PaymentRuntimeConfig): Promise<VerifiedPaymentTransaction | null> {
    const template = getPaymentRuntimeValue(config, "DQ_PAYPLY_QUERY_URL", "PAYPLY_QUERY_URL");
    ...
    const response = await fetchSafeOutboundUrl(url, { headers, signal: AbortSignal.timeout(15_000) });
    const payload = (await response.json().catch(() => ({}))) as Record<string, unknown>;
    if (!response.ok) throw new BillingInputError(readError(payload, "PayPly 交易查询失败"), response.status >= 500 ? 502 : 400);
    const status = String(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_STATUS_FIELD", ["status", "tradeStatus", "paymentStatus", "data.status", "data.tradeStatus"]) || "").toLowerCase();
    return {
        ...
        orderId: clean(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_ORDER_ID_FIELD", ["orderId", "data.orderId", "metadata.orderId", "metadata.dqOrderId"]), 120),
        orderNo: clean(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_ORDER_NO_FIELD", ["orderNo", "outTradeNo", "out_trade_no", "data.orderNo", "metadata.dqOrderNo"]), 120),
        providerTradeId: clean(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_TRADE_ID_FIELD", ["providerTradeId", "tradeId", "trade_no", "transactionId", "data.tradeId"]), 160),
        providerPaymentId: clean(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_PAYMENT_ID_FIELD", ["providerPaymentId", "paymentId", "payment_id", "transactionId", "data.paymentId"]), 160),
        amountCents:
            optionalInteger(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_AMOUNT_CENTS_FIELD", ["amountCents", "data.amountCents"])) ??
            decimalToCents(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_AMOUNT_FIELD", ["amount", "totalAmount", "data.amount"])),
        currency: currency(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_CURRENCY_FIELD", ["currency", "data.currency"])),
        paidAt: optionalIso(readConfiguredPath(config, payload, "DQ_PAYPLY_QUERY_PAID_AT_FIELD", ["paidAt", "successTime", "data.paidAt"])),
        rawPayload: sanitizeJson(payload),
    };
}
...
function loadPrivateKey(config: PaymentRuntimeConfig, valueName: string, pathName: string) {
    ...
    return normalized.includes("-----BEGIN") ? normalized : `-----BEGIN ${pemLabel}-----\n${normalized.match(/.{1,64}/g)?.join("\n") || normalized}\n-----END ${pemLabel}-----`;
}

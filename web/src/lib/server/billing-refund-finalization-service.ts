...
async function markPaymentRefunded(payment: PaymentTransactionRecord, input: BillingRefundFinalizationInput, now: string, update: (payment: PaymentTransactionRecord) => Promise<PaymentTransactionRecord>) {
    return update({
        ...payment,
        status: "refunded",
